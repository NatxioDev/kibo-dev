        ...
        <ul className="flex flex-col">
          {transactions.map((transaction) => {
            ...
            return (
              <li
                key={transaction.id}
                className="flex items-start justify-between gap-3 border-b border-zinc-800 py-3 last:border-b-0"
              >
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-zinc-500">
                    {formatTransactionDate(transaction.date)}
                  </p>
                  <p className="mt-1 truncate text-sm font-medium text-zinc-50">
                    {merchant}
                  </p>
